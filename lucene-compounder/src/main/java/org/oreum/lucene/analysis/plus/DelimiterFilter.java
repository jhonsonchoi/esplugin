 ...
 * </li>
 * </ul>
 * One use for {@link DelimiterFilter} is to help match words with different
 ...
 */
public final class DelimiterFilter extends TokenFilter {
    ...
    private final DelimiterIterator iterator;
     ...
     */
    public DelimiterFilter(TokenStream in, byte[] charTypeTable, int configurationFlags, CharArraySet protWords) {
        super(in);
        this.flags = configurationFlags;
        this.protWords = protWords;
        this.iterator = new DelimiterIterator(
     ...
     * @param protWords If not null is the set of tokens to protect from being delimited
     */
    public DelimiterFilter(TokenStream in, int configurationFlags, CharArraySet protWords) {
        this(in, DelimiterIterator.DEFAULT_WORD_DELIM_TABLE, configurationFlags, protWords);
    }
    ...
    public boolean incrementToken() throws IOException {
        while (true) {
            if (!hasSavedState) {
                ...
                if (iterator.end == DelimiterIterator.DONE && !has(PRESERVE_ORIGINAL)) {
    ...
    private void buffer() {
        if (termAttribute.length() < 2) return;
    ...
    private void generatePart(boolean isSingleWord) {
        ...
        posIncAttribute.setPositionIncrement(1/*position(false)*/);
